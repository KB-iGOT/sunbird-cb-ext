...
public class ExtendedOrgServiceImpl implements ExtendedOrgService {
        ...
        try {
            ...
            if (StringUtils.isEmpty(orgId)) {
                ...
                if (Constants.STATE.equalsIgnoreCase(orgType) || Constants.MINISTRY.equalsIgnoreCase(orgType)) {
                    ...
                    OrgHierarchy existingDBRecord = orgRepository.findByOrgNameAndParentMapId((String) requestData.get(Constants.CHANNEL), (String) requestData.get(Constants.PARENT_MAP_ID));
                    if (existingDBRecord == null) {
                        channelName = prepareChannelName((String) requestData.get(Constants.PARENT_MAP_ID), requestData);
                        channelName = channelName + requestData.get(Constants.CHANNEL);
                        ...
                        if (StringUtils.isEmpty(existingDBRecord.getSbRootOrgId())) {
                            existingDBRecord.setSbRootOrgId(fetchRootOrgId((String) requestData.get(Constants.PARENT_MAP_ID)));
                        }
            ...
            if (orgCreatedWithNewChannel) {
                ...
                if (!Constants.STATE.equalsIgnoreCase(orgType) && !Constants.MINISTRY.equalsIgnoreCase(orgType)) {
                    ...
                    if (!StringUtils.isEmpty(sbRootOrgid)) {
                        ...
                        updateRequest.put(Constants.SB_ROOT_ORG_ID, fetchRootOrgId((String) requestData.get(Constants.PARENT_MAP_ID)));
                    }
                } else {
                ...
                if (!StringUtils.isEmpty((String) requestData.get(Constants.MAP_ID))) {
                    ...
                    if (ObjectUtils.isEmpty(updateRequest.get(Constants.SB_ROOT_ORG_ID))) {
                        orgRepository.updateOrgIdForChannel(channelName, (String) updateRequest.get(Constants.SB_ORG_ID));
                    } else {
                        orgRepository.updateSbOrgIdAndSbOrgRootIdForChannel(channelName, (String) updateRequest.get(Constants.SB_ORG_ID), (String) updateRequest.get(Constants.SB_ROOT_ORG_ID));
                    }
                } else {
                    OrgHierarchy newOrg = new OrgHierarchy(orgName, channelName, mapId, (String) updateRequest.get(Constants.PARENT_MAP_ID));
    ...
    public SBApiResponse orgExtSearch(Map<String, Object> request) throws Exception {
        ...
        try {
            ...
            int limit = Optional.ofNullable((Integer) requestData.get(Constants.LIMIT)).filter(l -> l > 0) // Ensure positive value
                    .orElse(20); // Default to 20 if null or <= 0
            int offset = Optional.ofNullable((Integer) requestData.get(Constants.OFFSET)).filter(o -> o >= 0) // Ensure non-negative value
    ...
    private String checkOrgExist(String channel, String userToken) {
        ...
        String url = configProperties.getSbUrl() + configProperties.getSbOrgSearchPath();
        Map<String, Object> apiResponse = outboundService.fetchResultUsingPost(url, searchRequestBody, headers);
        if (Constants.OK.equalsIgnoreCase((String) apiResponse.get(Constants.RESPONSE_CODE))) {
    ...
    private String createOrgInSunbird(Map<String, Object> request, String channel, String userToken) {
        ...
        Map<String, Object> apiResponse = outboundService.fetchResultUsingPost(url, request, headers);
        if (Constants.OK.equalsIgnoreCase((String) apiResponse.get(Constants.RESPONSE_CODE))) {
            Map<String, Object> result = (Map<String, Object>) apiResponse.get(Constants.RESULT);
            logger.info(String.format("Org onboarded successfully for Name: %s, with orgId: %s", channel, result.get(Constants.ORGANIZATION_ID)));
    ...
    public Map<String, Object> getOrgDetails(List<String> orgIds, List<String> fields) {
        ...
        headers.put(Constants.CONTENT_TYPE, Constants.APPLICATION_JSON);
        Map<String, Object> apiResponse = outboundService.fetchResultUsingPost(configProperties.getSbUrl() + configProperties.getSbOrgSearchPath(), request, headers);
    ...
    public SBApiResponse createOrgForUserRegistration(Map<String, Object> request) {
        ...
        try {
            ...
            if (!StringUtils.isEmpty(orgId)) {
                if (dbUpdateRequired) {
                    OrgHierarchy existingDBRecord = orgRepository.findByChannel((String) requestData.get(Constants.CHANNEL));
                    if (StringUtils.isBlank(existingDBRecord.getSbOrgId())) {
                        ...
                        logger.error(String.format("Failed to update rootOrg details. RootOrg is already available in DB record. Existing: %s, NewValue: %s", existingDBRecord.getSbOrgId(), orgId));
                    }
                }
                response.getResult().put(Constants.ORGANIZATION_ID, orgId);
                response.getResult().put(Constants.RESPONSE, Constants.SUCCESS);
    ...
    private String createMapId(Map<String, Object> requestData) {
        ...
        if (CollectionUtils.isNotEmpty(existingOrgList)) {
            ...
            if (Constants.ENABLED.equalsIgnoreCase(configProperties.getMapIdCounterEnabled())) {
                ...
                List<Integer> numbers = existingOrgList.stream().map(OrgHierarchy::getMapId)                // Extract mapId
    ...
    public SBApiResponse orgExtSearchV2(Map<String, Object> request) {
        ...
        try {
            ...
            if (searchFilters.containsKey(Constants.IDENTIFIER)) {
                ...
                orgList = orgRepository.searchOrgWithHierarchy((String) searchFilters.get(Constants.ORG_NAME));
            }
            ...
            if (CollectionUtils.isEmpty(orgList)) {
                ...
                Set<String> l1MapIdSet = orgList.stream().map(OrgHierarchy::getL1MapId).filter(l1MapId -> Objects.nonNull(l1MapId)).collect(Collectors.toSet());

                List<OrgHierarchy> parentList = orgRepository.searchOrgForL1MapId(l1MapIdSet);
                Map<String, OrgHierarchy> parentListMap = parentList.stream().collect(Collectors.toMap(OrgHierarchy::getMapId, orgHierarchy -> orgHierarchy));
                for (OrgHierarchy org : orgList) {
        ...
        try {
            ...
            if (!StringUtils.isEmpty(orgId)) {
                ...
                if (StringUtils.isBlank(parentOrg.getSbRootOrgId()) && !StringUtils.isEmpty(sbRootOrgId)) {
                    orgRepository.updateSbOrgIdAndSbOrgRootIdForChannel(parentOrg.getChannel(), orgId, sbRootOrgId);
                } else {
    ...
    private Map<String, Object> updateOrgDetailsToDB(String authUserToken, OrgHierarchyInfo orgHierarchyInfo, Map<String, Object> orgRequest) {
        ...
        return outboundService.fetchResultUsingPatch(configProperties.getSbUrl() + configProperties.getUpdateOrgPath(), updateRequest, headerValues);
    }
    ...
    public SBApiResponse getNodalOfficer(String orgId) {
        ...
        try {
            ...
            finalQuery.must(QueryBuilders.termQuery(Constants.ROLE_PATH, Constants.MDO_LEADER));
